...
test.cb('getSecret: happy path', t => {
  td.when(AWS.SSM.prototype.getParameters(td.matchers.anything())).thenReturn({
    promise: () =>
      Promise.resolve({
        Parameters: [
          {
...
test.cb('getSecret: requests decryption', t => {
  td.when(
    AWS.SSM.prototype.getParameters(td.matchers.contains({ WithDecryption: true }))
...
test('setSecret: happy path', t => {
  td.when(
    AWS.SSM.prototype.putParameter({
      Name: 'name',
      Value: 'value',
...
test.cb('setSecret: error bubbles up', t => {
  const error = {};
  td.when(AWS.SSM.prototype.putParameter(td.matchers.anything())).thenReturn({
    promise: () => Promise.reject(error),
  });
