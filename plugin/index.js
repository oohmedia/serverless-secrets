...
class ServerlessSecrets {
  constructor(serverless, options) {
    ...
    this.commands = {
      secrets: {
        usage:
          'CLI interface for Serverless Secrets. Subcommands: set, get, list-remote, validate.',
        commands: {
          set: {
            lifecycleEvents: ['set'],
            ...
            options: {
              ...
              region: {
                usage:
                  'AWS region where secret will be stored; region will fallback to configured (or default) `providerOptions`',
                shortcut: 'r',
              },
              key: {
          ...
          get: {
            ...
            options: {
              ...
              region: {
                usage:
                  'AWS region where secret is stored; if not supplied, region will fallback to configured (or default) `providerOptions`',
                shortcut: 'r',
              },
            },
          },
  ...
  setSecret() {
    ...
    storageProvider
      .setSecret(
        ...
        customKey || this.options.key || defaultKey
      )
      .then(() =>
        this.serverless.cli.log(
          `Secret ${this.options.name} stored! Remember to add it to your environmentSecrets section(s) to retrieve it!`
        )
      );
  }
  ...
  getSecret() {
    const storageProvider = this.getStorageProvider();
    storageProvider.getSecret(this.options.name).then(data => {
  ...
  deleteSecret() {
    const storageProvider = this.getStorageProvider();
    storageProvider
      .deleteSecret(this.options.name)
  ...
  packageSecrets() {
    ...
    return this.validateSecrets().then(() => this.setAdditionalEnvironmentVariables());
  }
  ...
  setAdditionalEnvironmentVariables() {
    ...
    Object.keys(functions).forEach(functionName => {
      ...
      Object.assign(
        ...
        this.config.environments.$global,
        this.config.environments[functionName]
      );
    });
  }
  ...
  validateSecrets() {
    ...
    const missingSecretsPromise = storageProvider.listSecrets(rootSecretPaths).then(secrets => {
      ...
      const missingProviderEnvironmentSecretsGroup = this.findAllEnvironmentSecretsMissingRemotely(
      ...
      return [
        this.constructMissingRemoteSecretsErrorMessage(
          missingProviderEnvironmentSecretsGroup,
          null
        ),
      ]
        .concat(
    ...
    const providerCollisionGroup = this.findAllEnvironmentSecretsDuplicatedInEnvironment(
      provider.environmentSecrets,
  ...
  constructCollisionsErrorMessage(
    collisions,
    environmentSecretsFunctionName,
    environmentFunctionName
  ) {
    // null/undefined function name means provider
    if (!collisions.length) return '';
    const envSecretsName = `${
      environmentSecretsFunctionName
        ? `function ${environmentSecretsFunctionName}`
        : 'the provider section'
    }`;
    const envName = `${
      environmentFunctionName ? `function ${environmentFunctionName}` : 'the provider section'
    }`;
